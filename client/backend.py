...
class User:
    ...
    def get_user_exists_by_username(username: str):
        response = requests.get(config["request_url"] + "get_user_exists_by_username", params={"username": username})
        return response.json()

    @staticmethod
    def get_user_exists_by_id(user_id: int):
        return requests.get(config["request_url"] + "get_user_exists_by_id", params={"user_id": user_id}).json()

    @staticmethod
    def add_user(userdata: AddUser):
        output = requests.post(config["request_url"] + "add_user", json=userdata.model_dump())
        output.raise_for_status()
        return True

    @staticmethod
    def get_all_userdata():
        return requests.get(config["request_url"] + "get_all_userdata").json()

    @staticmethod
    def get_userdata(user_id: int):
        response = requests.get(config["request_url"] + "get_userdata", params={"user_id": user_id})
    ...
    def get_userdata_by_username(username: str):
        response = requests.get(config["request_url"] + "get_userdata_by_username", params={"username": username})
        if response.status_code != 200:
    ...
    def get_username_list():
        return requests.get(config["request_url"] + "get_username_list").json()

    @staticmethod
    def get_transaction_list(user_id: int):
        return ([
            RawTransactionData(**transaction) for transaction in
            requests.get(config["request_url"] + "get_transaction_list", params={"user_id": user_id}).json()
        ])

    @staticmethod
    def generate_transaction(current_money: float, transaction_details: TransactionField):
        params = {"current_money": current_money}
        return requests.post(config["request_url"] + "generate_transaction",
    ...
    def set_saldo(user_id: int, transaction_details: TransactionField):
        ...
        return requests.put(config["request_url"] + "set_saldo",
                            params={"user_id": user_id}, json=data).json()

    @staticmethod
    def delete_user(user_id: int):
        response = requests.delete(config["request_url"] + "delete_user", params={"user_id": user_id})
        return response.status_code == 200

    @staticmethod
    def rename_user(user_id: int, new_username: str):
        params = {"user_id": user_id, "new_username": new_username}
        response = requests.put(config["request_url"] + "rename_user", params=params)
...
def filter_list(search: str, seq: list[str], case_sensitive: bool = False, order_alphabetically: bool = True) -> list:
    ...
    for item in seq:
        ...
        elif (search in item) if case_sensitive else (search.lower() in item.lower()):
            result.append(item)
